...
export default function DogSchemaUpdateForm(props) {
  const {
    id,
    dogSchema,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...dogSchemaRecord };
    setDogName(cleanValues.dogName);
    setDogSize(cleanValues.dogSize);
    setDogYears(cleanValues.dogYears);
    setDogMonths(cleanValues.dogMonths);
    setDogWeight(cleanValues.dogWeight);
    setDogAge(cleanValues.dogAge);
    setAtThePark(cleanValues.atThePark);
    setStartTimeAtPark(cleanValues.startTimeAtPark);
    setDogAgeGroup(cleanValues.dogAgeGroup);
    setDogSex(cleanValues.dogSex);
    setDogLikes(cleanValues.dogLikes ?? []);
    setCurrentDogLikesValue(undefined);
    setDogDislikes(cleanValues.dogDislikes ?? []);
    setCurrentDogDislikesValue(undefined);
    setDogParks(cleanValues.dogParks);
    setOwnerId(cleanValues.ownerId);
    setFriendsArray(cleanValues.friendsArray ?? []);
    setCurrentFriendsArrayValue(undefined);
    setErrors({});
  };
  const [dogSchemaRecord, setDogSchemaRecord] = React.useState(dogSchema);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(DogSchema, id) : dogSchema;
      setDogSchemaRecord(record);
    };
    queryData();
  }, [id, dogSchema]);
  React.useEffect(resetStateValues, [dogSchemaRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            DogSchema.copyOf(dogSchemaRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "DogSchemaUpdateForm")}
    >
      <TextField
        label="Dog name"
        isRequired={true}
